...
from pacman.algorithms import dpop
from pacman.algorithms.dpop import DpopMessage
from pacman.dcop.objects import Variable
from pacman.dcop.relations import NAryMatrixRelation, AsNAryFunctionRelation
...
class TestSmartLightSample:
    def test_4variables(self):
        ...
        joined = pacman.dcop.relations.join(scene_rel, cost_l3)

        assert joined(9, 6, 0, 5) == 0

        assert joined(3, 6, 0, 5) == 10000

        util = pacman.dcop.relations.projection(joined, l3, "min")
