...
import pacman.algorithms
import pacman.dcop.relations
import pacman.utils
import pacman.utils.various
from pacman.infrastructure.computations import Message
from pacman.dcop.objects import VariableDomain, Variable, VariableWithCostFunc
from pacman.dcop.relations import (
...
class GenerateAssignementTestCase(unittest.TestCase):
    def test_generate_1var(self):
        ...
        ass = list(pacman.dcop.relations.generate_assignment([x1]))
    ...
    def test_generate_1var_generator(self):
        ...
        ass = pacman.dcop.relations.generate_assignment([x1])
    ...
    def test_generate_2var(self):
        ...
        ass = list(pacman.dcop.relations.generate_assignment([x1, x2]))
        print(ass)
    ...
    def test_generate_3var(self):
        ...
        ass = list(pacman.dcop.relations.generate_assignment([x1, x2, x3]))
...
class GenerateAssignementAsDictTestCase(unittest.TestCase):
    def test_generate_1var(self):
        ...
        ass = list(pacman.dcop.relations.generate_assignment_as_dict([x1]))
    ...
    def test_generate_2var(self):
        ...
        ass = list(pacman.dcop.relations.generate_assignment_as_dict([x1, x2]))
        print(ass)
        self.assertEqual(len(ass), len(x1.domain) * len(x2.domain))
...
class FindArgOptimalTestCase(unittest.TestCase):
    def test_findargmax(self):
        ...
        x1 = Variable("x1", ["a", "b", "c"])
        u1 = pacman.dcop.relations.NAryMatrixRelation(
            [x1], np.array([2, 4, 8], np.int8)
        )

        # take the projection of u1 along x1
        m, c = pacman.dcop.relations.find_arg_optimal(x1, u1, mode="max")
    ...
    def test_findargmin(self):
        ...
        # take the projection of u1 along x1
        m, c = pacman.dcop.relations.find_arg_optimal(x1, u1, mode="min")
    ...
    def test_findargmin_fct(self):
        ...
        m, c = pacman.dcop.relations.find_arg_optimal(v1, f1, mode="min")
    ...
    def test_findargmin_several_values(self):
        ...
        values, c = pacman.dcop.relations.find_arg_optimal(v1, f1, mode="min")
...
def test_check_type_by_string():
    pacman.algorithms.is_of_type_by_str(2, "int")
    pacman.algorithms.is_of_type_by_str(2.5, "float")
    pacman.algorithms.is_of_type_by_str("foo", "str")


def test_check_type_by_string_invalid_type():
    assert not pacman.algorithms.is_of_type_by_str(2, "str")
    assert not pacman.algorithms.is_of_type_by_str("2.5", "float")
    assert not pacman.algorithms.is_of_type_by_str(0.25, "int")


def test_is_valid_param_value():
    param_def = pacman.algorithms.AlgoParameterDef("p", "str", ["a", "b"], "b")

    assert "b" == pacman.algorithms.check_param_value("b", param_def)
    assert "a" == pacman.algorithms.check_param_value("a", param_def)

    with pytest.raises(ValueError):
        pacman.algorithms.check_param_value("invalid_value", param_def)
...
def algo_param_defs():
    yield [
        pacman.algorithms.AlgoParameterDef("param1", "str", ["val1", "val2"], "val1"),
        pacman.algorithms.AlgoParameterDef("param2", "int", None, None),
    ]


def test_algo_parameters_all_defaults(algo_param_defs):
    default_params = pacman.algorithms.prepare_algo_params({}, algo_param_defs)
...
def test_algo_parameters_with_valid_str_param(algo_param_defs):
    params = pacman.algorithms.prepare_algo_params({"param1": "val2"}, algo_param_defs)
...
def test_algo_parameters_with_valid_int_param(algo_param_defs):
    ...
    params = pacman.algorithms.prepare_algo_params({"param2": 5}, algo_param_defs)
    assert "param2" in params
    assert params["param2"] == 5


def test_algo_parameters_with_all_params(algo_param_defs):
    params = pacman.algorithms.prepare_algo_params(
...
def test_algo_parameters_with_int_conversion(algo_param_defs):
    params = pacman.algorithms.prepare_algo_params({"param2": "10"}, algo_param_defs)
...
def test_algo_parameters_with_invalid_param(algo_param_defs):
    with pytest.raises(ValueError):
        pacman.algorithms.prepare_algo_params({"invalid_param": "foo"}, algo_param_defs)


def test_algo_parameters_with_invalid_value(algo_param_defs):
    with pytest.raises(ValueError):
        pacman.algorithms.prepare_algo_params(
