...
"""
.. _pacman_commands_agent:

pacman agent
============

``pacman agent`` runs one or several standalone agent(s).
...
separately  using the `:ref:`pacman_commands_orchestrator` command.
...
**Command:** :ref:`pacman_commands_orchestrator`
...
``--restart``
  ...
  stopped. Useful when running `pacman agent` as daemon on a remote machine.
...
Running a single agent on port 9001, with an ui-server on port 10001::
    ...
    pacman -v 3 agent -n a1 --orchestrator 127.0.0.1:9000 --uiport 10001

Running 5 agents, listening on port 9011 - 9016 (without ui-server)::

    pacman -v 3 agent -n a1 a2 a3 a4 a5 -p 9011 --orchestrator 127.0.0.1:9000
...
logger = logging.getLogger("pacman.cli.agent")
force_stopped = False
agents = []
...
def set_parser(subparsers):
    ...
    parser.add_argument(
        ...
        "running `pacman agent` as daemon on a remote "
        "machine.",
    )
    parser.add_argument(
        "--delay",
