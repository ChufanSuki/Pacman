...
from pacman.dcop.objects import AgentDef, create_binary_variables
from pacman.dcop.objects import BinaryVariable
from pacman.dcop.relations import Constraint
from pacman.infrastructure.Events import event_bus
from pacman.infrastructure.communication import Messaging, \
    CommunicationLayer, UnreachableAgent
from pacman.infrastructure.computations import MessagePassingComputation, \
    build_computation
from pacman.infrastructure.discovery import Discovery, UnknownComputation, \
    UnknownAgent, _is_technical
from pacman.infrastructure.ui import UiServer
from pacman.reparation import create_computation_hosted_constraint, \
...
class Agent(object):
    ...
    def __init__(self, name,
        ...
        self.logger = logging.getLogger('pacman.agent.' + name)
    ...
    def run(self, computations: Optional[Union[str, List[str]]] = None):
        ...
        for c in list(self._computations.values()):
            if computations is None:
                if c.is_running:
                    self.logger.debug(
                        f'Do not start computation {c.name}, already '
                        'running')
                else:
                    c.start()
    ...
    def metrics(self):
        ...
        own_computations = {c.name for c in
                            self.computations(include_technical=True)}
        m = {
            'count_ext_msg': {k: v
...
from pacman.computations_graph import constraints_hypergraph as chg
...
class ResilientAgent(Agent):
    ...
    def __init__(self, name: str, comm: CommunicationLayer,
        ...
        if replication is not None:
            ...
            algo_module = import_module('pacman.replication.{}'
                                        .format(replication))
    ...
    def _on_repair_computation_finished(self, computation: str):
        ...
        if all(c.status == 'finished'
            ...
            repair_metrics = {'count_ext_msg': {}, 'size_ext_msg': {},
                              'cycles': {}}

            for c in self._repair_computations.values():
                c_name = c.computation.name
                if c_name in metrics['count_ext_msg']:
                    repair_metrics['count_ext_msg'][c_name] = \
                    metrics['count_ext_msg'][c_name]
                else:
                    repair_metrics['count_ext_msg'][c_name] = 0
                if c_name in metrics['size_ext_msg']:
                    repair_metrics['size_ext_msg'][c_name] = \
                    metrics['size_ext_msg'][c_name]
                else:
...
class RepairComputation(MessagePassingComputation):
    ...
    def __init__(self, agent: ResilientAgent):
        ...
        self.agent = agent
        self.logger = logging.getLogger('pacman.agent.repair.' + agent.name)
        self._handlers = {
