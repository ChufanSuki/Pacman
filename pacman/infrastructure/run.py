...
from pacman.algorithms import AlgorithmDef, load_algorithm_module
...
def solve(dcop: DCOP,
    ...
    algo_def: string or AlgorithmDef object
        ...
        the package `pacman.algorithms`, and this algorithm is used with it's
    ...
    distribution: either a Distribution object or a string
        ...
        in the package pacman.distribution. This module is the loaded and used
    ...
    graph: either a string a ComputationGraph object
        ...
        the `pacman.computations_graph` package ; the module is loaded and
    ...
    if graph is None:
        graph_module = import_module('pacman.computations_graph.{}'.
                                     format(algo_module.GRAPH_TYPE))
        graph = graph_module.build_computation_graph(dcop)

    elif isinstance(graph, str):
        graph_module = import_module('pacman.computations_graph.'+graph)
        graph = graph_module.build_computation_graph(dcop)

    if isinstance(distribution, str):
        distrib_module = import_module('pacman.distribution.' + distribution)
